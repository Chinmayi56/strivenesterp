...
export const ResetPasswordPage: React.FC = () => {
  ...
  const [newPassword, setNewPassword] = useState("NewAdminPassword123!");
  const [confirmPassword, setConfirmPassword] = useState("NewAdminPassword123!");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!resetToken.trim()) {
      setErrorMsg("Please provide a valid password reset token.");
      return;
    }
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.message || "Failed to reset password. Invalid or expired token.";
      setErrorMsg(msg);
      showError(msg);
    } finally {
      setLoading(false);
  ...
  return (
    <AuthLayout subtitle="Set your new secure password">
        ...
        {success ? (
          <div className="space-y-4 text-center py-2 animate-fadeIn">
            ...
            <div className="space-y-1">
              <h3 className="text-sm font-bold text-slate-100">Password Reset Completed</h3>
