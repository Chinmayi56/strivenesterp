...
export const MyDocumentModal: React.FC<Props> = ({ isOpen, onClose, onSuccess }) => {
  const [documentType, setDocumentType] = useState("ID Proof");
  const [documentName, setDocumentName] = useState("");
  const [fileName, setFileName] = useState("");
  const [fileUrl, setFileUrl] = useState("");
  const [fileSize, setFileSize] = useState(1024 * 500);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!documentName || !fileName) {
      showError("Please specify a document name and select a file.", "Validation Error");
      return;
    }
    ...
    try {
      ...
      await employeeService.uploadMyDocument(payload);
      showSuccess(`Document '${documentName}' uploaded successfully.`, "Document Uploaded");
      onSuccess();
      onClose();
    } catch (err: any) {
      showError(err.message || "Failed to upload document.", "Upload Error");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-100">Upload Personal Document</h3>
              <p className="text-xs text-slate-400">Upload identity cards, tax forms, or certifications.</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 text-xs text-slate-200">
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-slate-100 focus:outline-none focus:border-indigo-500 font-mono"
            >
              ...
              <option value="Tax Document">Tax Form (W2 / 1099)</option>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Driver's License - 2026"
            ...
            <div className="relative border-2 border-dashed border-slate-800 hover:border-slate-700 bg-slate-950/60 rounded-2xl p-6 text-center transition-colors">
              ...
              {fileName ? (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500">PDF, PNG, JPG (Max 10MB)</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-4 mt-4 border-t border-slate-800 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold transition-all shadow-lg shadow-indigo-500/20 disabled:opacity-50 flex items-center gap-2"
            >
